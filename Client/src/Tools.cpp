...
namespace Tools
{
    std::pair<std::shared_ptr<sf::Texture>, sf::Sprite> loadSprite(const std::string &path, float _scale) {
        ...
        uint _sprite_size = texture->getSize().x;
        sprite.setTexture(*texture);
        sprite.setScale(_scale, _scale);
        sprite.setOrigin(_sprite_size / 2, _sprite_size / 2);
        return std::make_pair(texture, sprite);
    }
